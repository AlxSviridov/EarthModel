...
- 2026-08-30 — Rebuilt the city marker as a map pin after user feedback that the label "zooms together with globe ... looks too massive". The cause was drei's `distanceFactor`, which scales a label by 1/distance: the city name swung more than sixteenfold across the camera range, roughly 32 px at closest zoom and under 3 px at the full-orbit preset. It was broken at both ends, and the responsive audits never caught it because they measure computed CSS font-size, which is blind to drei's transform. The Sun and season labels carried the same prop at different values and are now constant-size too, matching the sky dome, which had always omitted it.
